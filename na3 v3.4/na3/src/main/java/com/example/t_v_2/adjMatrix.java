...
public class adjMatrix {
    ...
    public void printPanel(JPanel jp, HashMap<String, Point> pp) {
        for(String i: pp.keySet()){
            Graphics g;
            g=jp.getGraphics();
            g.setColor(Color.black);
            g.fillArc(pp.get(i).getX() - 10, pp.get(i).getY() - 10, 20, 20, 0, 360);
            jp.getGraphics().drawArc(pp.get(i).getX() - 10, pp.get(i).getY() - 10, 20, 20, 0, 360);//画圆
            jp.getGraphics().drawString(i, pp.get(i).getX() - 5, pp.get(i).getY() + 20);//画name
        ...
        for(int i = 0; i < verNum; i ++)
            for(int j = 0; j < verNum; j ++){
                if(edges[i][j] != noEdge){
                    jp.getGraphics().drawLine(pp.get(getVertices(i)).getX(), pp.get(getVertices(i)).getY(), pp.get(getVertices(j)).getX(), pp.get(getVertices(j)).getY());
                    jp.getGraphics().drawString(String.valueOf(edges[i][j]), (pp.get(getVertices(i)).getX() + pp.get(getVertices(j)).getX()) / 2, (pp.get(getVertices(i)).getY() +pp.get(getVertices(j)).getY()) / 2);
                }
            }

    }

    public void printPanelname(JPanel jp, HashMap<String, Point> pp) {
        for(String i: pp.keySet()){
            Graphics g;
            g=jp.getGraphics();
            g.setColor(Color.black);
            ...
            for (Spot spot : spots) {
                if(spot.getSpid().equals(i)){
                    spot1=spot;
                    if(spot1==null){
                        spot1.setSpname(name);
                    }
                }
            }

            jp.getGraphics().drawString(/*i + ":" + */spot1.getSpname(), pp.get(i).getX() - 5, pp.get(i).getY() + 20);//画name
            //System.out.println(i);
